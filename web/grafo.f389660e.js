...
function passo(){
  ...
  return ns.length ? energia / ns.length : 0;
}
...
let LARG = 0, ALT = 0;
addEventListener('resize', () => { LARG = 0; redesenha(); });
function desenha(){
...
let quieto = 0, rodando = false;
function laco(){
  rodando = true;
  if (pausado){ rodando = false; return; }
  quieto = passo() < 0.02 ? quieto + 1 : 0;
  desenha();
  if (quieto > 34){ rodando = false; return; }   // assentou: para de queimar CPU
  requestAnimationFrame(laco);
}
function acorda(){ quieto = 0; if (!rodando && !pausado){ rodando = true; requestAnimationFrame(laco); } }
function redesenha(){ if (!rodando) desenha(); }
...
cv.addEventListener('pointermove', ev => {
  ...
  if (panDe){ cam.x += (mx-panDe.x)/cam.z; cam.y += (my-panDe.y)/cam.z; panDe = {x:mx,y:my};
    redesenha(); return; }
  const antes = hover;
  const n = noEm(mx,my); hover = n?n.id:null; cv.style.cursor = n?'pointer':'grab';
  if (hover !== antes) redesenha();
});
